...
export function TagDistributionChart({
    ...
    const handleTagClick = (item: DistributionData) => {
        ...
        if (item.id === 'others') {
            params.append('has_tags', 'false')
        } else {
            params.append('tag', item.id)
        }
    ...
    return (
        <Card className={cn(
            ...
            <CardHeader className="pb-0 pt-6 px-6">
                ...
                {description && <CardDescription className="text-[10px] font-bold uppercase tracking-wider opacity-60">{description}</CardDescription>}
            </CardHeader>
            <CardContent className="h-auto min-h-[320px] flex flex-col sm:flex-row items-center justify-between p-4 sm:p-5 lg:p-6 gap-2 sm:gap-4 lg:gap-8">
                {isLoading ? (
                    ...
                    <ChartEmptyState 
                        ...
                        height="300px" 
                        title={title}
                        description={`Nenhum dado de ${title.toLowerCase()} encontrado no período.`}
                        icon={TagIcon}
                    />
                ) : (
                    <>
                        ...
                        <div className="w-full sm:w-[160px] md:w-[150px] lg:w-[220px] xl:w-[240px] space-y-0.5 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar mt-4 sm:mt-0 shrink-0">
                            {data.map((item, index) => (
                                <div 
                                    ...
                                    className="flex items-center justify-between py-1.5 px-3 rounded-xl hover:bg-muted/30 transition-colors group cursor-pointer"
                                >
